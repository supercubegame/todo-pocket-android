...
def main():
    ...
    before=stored_state(); paths=adb("shell","find","/storage/emulated/0/Download","-type","f","-name","PocketTodo-backup.ptodo").strip().splitlines()
    check(len(paths)==1 and paths[0]=="/storage/emulated/0/Download/PocketTodo-backup.ptodo","locate disposable exported backup fixture")
    check(adb("exec-out","cat",paths[0],binary=True).startswith(b"POCKET_TODO_BACKUP\n1\n"),"verify exported fixture before corruption")
    corrupt=Path(os.environ["RUNNER_TEMP"])/"corrupt.ptodo"; corrupt.write_bytes(b"broken backup\n"); adb("push",str(corrupt),paths[0])
    check(adb("exec-out","cat",paths[0],binary=True)==b"broken backup\n","read back corrupt fixture bytes")
